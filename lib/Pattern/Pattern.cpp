...
byte Pattern::outputStepByte(){
    long trigger_millis = millis();
    if(sequence[_position] == true && _triggered_step != _position){   
        _triggered_step = _position;
        _previous_trigger_millis = trigger_millis;
        return 255;
    }
    if(trigger_millis - _previous_trigger_millis >= TRIGGER_INTERVAL){
        return 0;
    }
}
